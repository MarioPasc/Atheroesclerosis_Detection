...
class HoldOut:
    def __init__(self, csv_path, output_dir='./data/holdout'):
    ...
    def save_splits(self, train_df, val_df, test_df):
        for split_name, df in zip(['train', 'val', 'test'], [train_df, val_df, test_df]):
            ...
            split_df['Frame_path'] = split_df.apply(
                lambda row: row['SelectedFramesLesionVideo'] if row['Lesion'] else row['SelectedFramesNonLesionVideo'], axis=1
            )
            split_df['Groundtruth_path'] = split_df['GroundTruthFile'].apply(lambda x: x if isinstance(x, str) and x.startswith('/home/') else 'nolesion')
